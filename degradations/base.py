    ...
    def lift(
        ...
        *,
        ...
        if lift_mode not in ("bilinear", "nearest"):
            raise ValueError(
                f"{self.__class__.__name__} does not define {lift_mode!r} lifting; "
                "use bilinear/nearest or physical mode."
            )
        return resize_up(y, scale, mode=lift_mode, target_size=target_size)

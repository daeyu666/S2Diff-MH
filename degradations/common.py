...
def validate_scale(x: torch.Tensor, scale: int) -> None:
    ...
    if h % scale != 0 or w % scale != 0:
        raise ValueError(f"Spatial size {(h, w)} must be divisible by scale={scale}")
...
def area_average_adjoint(y: torch.Tensor, scale: int) -> torch.Tensor:
    ...
    if scale == 1:
        return y
    c = y.shape[1]
    kernel = torch.ones((c, 1, scale, scale), dtype=y.dtype, device=y.device) / float(scale * scale)
...
def assert_spatial_size(x: torch.Tensor, expected_size: Tuple[int, int], name: str) -> None:
    if tuple(x.shape[-2:]) != tuple(expected_size):
        raise RuntimeError(
            f"{name} produced spatial size {tuple(x.shape[-2:])}, expected {tuple(expected_size)}"
        )

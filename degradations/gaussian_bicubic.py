"""Legacy Gaussian-blur + bicubic-resize HSI degradation baseline."""
...
class GaussianBicubicDegradation(BaseDegradation):
    ...
    def __init__(self, scale_ratio: int = 4, sigma: float = 2.0, kernel_size: int = 5):
    ...
    def degrade_at(self, x: torch.Tensor, *, scale: int, strength: float) -> torch.Tensor:
        ...
        blurred = depthwise_psf(x, sigma_t, kernel_size=self.kernel_size)
        return resize_down(blurred, scale, mode="bicubic", antialias=True)

    def extra_repr(self) -> str:
        return super().extra_repr() + f", sigma={self.sigma}, kernel_size={self.kernel_size}"

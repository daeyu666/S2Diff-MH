...
class ProgressiveDegradation:
    """Frozen D_t/U_t/D~_t trajectory used by Innovation 1.

    With T=12 and scale_ratio=4:
    ...
    def __init__(
        ...
        self.stages = list(stages) if stages is not None else _unique_scales(operator.scale_ratio)
        if not self.stages or self.stages[0] != 1:
            raise ValueError("stages must start from 1")
        if self.stages[-1] != operator.scale_ratio:
            raise ValueError("last stage must equal operator.scale_ratio")
        if default_lift_mode is None:
            default_lift_mode = "normalized_adjoint" if operator.mode == "physical" else "bilinear"
        self.default_lift_mode = default_lift_mode

    def _validate_t(self, t: int) -> int:
        t = int(t)
        if t < 0 or t > self.total_steps:
            raise ValueError(f"t must lie in [0, {self.total_steps}], got {t}")
        return t
    ...
    def state(self, t: int) -> ProgressiveState:
        ...
        if t == 0:
            ...
            idx = min(((t - 1) * len(self.stages)) // self.total_steps, len(self.stages) - 1)
    ...
    def degrade_at(self, x: torch.Tensor, t: int) -> torch.Tensor:
        state = self.state(t)
        return self.operator.degrade_at(x, scale=state.scale, strength=state.strength)
    ...
    def state_at(self, x: torch.Tensor, t: int, *, lift_mode: Optional[str] = None) -> torch.Tensor:
        target_size = tuple(x.shape[-2:])
        y_t = self.degrade_at(x, t)
        return self.lift_at(y_t, t, target_size=target_size, lift_mode=lift_mode)
        ...
        *,
        ...
        if not torch.allclose(direct, progressive, atol=atol, rtol=rtol):
            ...
            raise AssertionError(
                f"Terminal degradation closure failed: max_abs_error={max_error:.6e}"
            )
